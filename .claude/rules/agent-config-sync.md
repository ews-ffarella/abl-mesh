...
The two tools spell the same scoping differently. **Cursor** reads `globs` /
`alwaysApply` and loads a rule only when matching files are open. **Claude
Code** reads `paths:` frontmatter: a rule without it loads into every session;
a rule with it loads lazily, when a matching file is read (ruled estate-wide
2026-08-31, fabien-context #57). So the trees cannot be byte-identical for
rules, and a human mirroring them by hand drifts.

The generator translates the Cursor `globs` into Claude `paths:` (always-on
rules stay frontmatter-free) and keeps a readable **Applies to** header for
humans.
...
Keep the set small. **The user-level rules already load in every session on
every project** (`~/.claude/CLAUDE.md`, `~/.cursor/rules/working-with-fabien.mdc`):
a repo rule that restates them is pure echo. Write a rule only for what is true
of *this* repo.
...
- **Never link into `.cursor/` itself** from a skill. Name such a path as plain
  text in backticks instead of a markdown link.
