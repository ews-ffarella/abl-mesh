...
def build_parser():
    p = argparse.ArgumentParser(description="Precompute HO polynomial coefficients (memmap tiled option) and optionally export GeoTIFF")
    p.add_argument("--raster", required=True, help="Input DEM (GeoTIFF)")
    p.add_argument("--degree", type=int, default=3, help="Polynomial degree for HO approximant (default 3)")
    p.add_argument("--do-precompute", action="store_true", help="Run precompute now (if not set, load provided memmap or exit)")
    p.add_argument("--memmap-path", default=None, help="If provided run tiled memmap precompute writing to this path (NumPy memmap).")
    p.add_argument("--load-memmap", default=None, help="If provided, load existing memmap into the approximant (no precompute).")
    p.add_argument("--tile-size", type=parse_tile_size, default=(256, 256), help="Tile size for tiled precompute, as 'rows,cols'")
    p.add_argument("--n-jobs", type=int, default=-1, help="Number of joblib workers when computing fits inside a tile (multiprocessing). -1 uses all cores.")
    p.add_argument("--use-tqdm", action="store_true", help="Show tqdm progress for the precompute.")
    p.add_argument("--export-coeffs", default=None, help="If set, export coefficients to GeoTIFF after precompute/load.")
    p.add_argument("--export-nodata", type=float, default=-9999.0, help="Nodata sentinel to use when exporting GeoTIFF.")
...
def main():
    ...
    ho = RasterHighOrderApproximant(rtopo, degree=args.degree, verbosity=args.verbosity, precompute=False)
    ...
    if args.load_memmap:
        ...
        if args.memmap_path:
            logging.info("Starting tiled memmap precompute -> %s (tile %s, n_jobs=%d)",
                         args.memmap_path, f"{args.tile_size[0]}x{args.tile_size[1]}", args.n_jobs)
            ho.precompute_all_coeffs(n_jobs=args.n_jobs, use_tqdm=args.use_tqdm,
                                     memmap_path=args.memmap_path, memmap_dtype="float32",
                                     tile_size=args.tile_size)
            logging.info("Memmap precompute finished: %s", args.memmap_path)
        else:
            logging.info("Starting in-memory precompute (may use a lot of memory for big rasters)...")
            ho.precompute_all_coeffs(n_jobs=args.n_jobs, use_tqdm=args.use_tqdm,
                                     memmap_path=None, tile_size=args.tile_size)
